...
- 多了一步使用 XMMatrixPerspectiveFovLH 计算 projection matrix 的过程，因为视口大小比例变了
...
### BoxApp::BuildRootSignature()

- 根签名可以理解为 shader 函数的声明，说明这个 shader 需要哪些输入数据资源，说明这些资源的格式、类型、存储等，md3dDevice->CreateRootSignature() 需要传入序列化的 ComPtr<ID3DBlob> 地址，ID3DBlob 是DX12 内建的一种存放较为庞大的二进制对象
- 创建根签名的步骤大致为：创建并设置根签名参数数组 -> 定义根签名，创建序列化根签名资源 -> 创建根签名
...
- 编译 Shader 获得 Blob 数据，然后定义输入元素信息，主要通过 D3DCompileFromFile 将 HLSL 代码编译为给定目标的字节码
...
- std::array 是具有固定大小的数组，除了有内置数组支持随机访问、效率高、存储大小固定等特点外，还支持迭代器访问、获取容量、获得原始指针等高级功能。而且它还不会退化成指针给开发人员造成困惑
- ID3D12Resource 封装 CPU 和 GPU 的通用化功能，以读取和写入物理内存或堆。 它包含用于组织和操作简单数据数组的抽象，以及针对着色器采样优化的多维数据。
- SubmeshGeometry 存储了 MeshGeometry 的一段子序列几何数据，用于多个对象几何数据使用同一个 vertex buffer 和 index buffer 的情况
...
- MeshGeometry 主要存储了名称、CPU 端的 Vertex/Index Buffer、GPU 端的 Vertex/Index Buffer、Uploader Vertex/Index Buffer、Data 的具体信息，比如 Stride/Format/BufferSize、一个 unordered_map 存储 SubmeshGeometry，以及返回 VBV、IBV 和释放资源函数
- 创建 Box 相关的几何数据，首先是创建顶点/索引数据并记录数据大小，然后创建 MeshGeometry，创建 Vertext/Index Blob/Buffer/Uploader 并设置大小/Format/Stride，然后创建 SubmeshGeometry 描述 Box 的 index/vertex 信息
...
- ZeroMemory 常被称为清零函数，用 0 来填充一块内存区域，PSO 主要需要设置 input layout、root signature、vs、ps、光栅器状态、混合状态、深度/模板状态、拓扑信息、渲染目标数量格式、采样，然后使用 CreateGraphicsPipelineState() 创建 PSO
...
- 整个 BoxApp 的初始化操作主要就是：D3DApp 初始化，如窗口、DX、Resize -> 创建 Constant Buffer 相关的描述符和buffer数据 -> 创建 root signature、Shader and Input layout -> 构建几何数据 -> 构建 PSO -> 执行 Commands 并等待初始化命令完成
...
- MSG 包含来自线程消息队列的消息信息，实际运行过程中就是处理消息和更新绘制场景
...
### BoxApp::Update(const GameTimer& gt)

- 更新操作主要是在获取相机位置，从而更新变换矩阵，然后更新 Constant Buffer
...
### BoxApp::Draw(const GameTimer& gt)

- 绘制过程主要通过 CommandList 绑定视区、裁剪区、清除 RTV 和 DSV、设置 RT 和 DS、绑定描述符堆、设置根签名、绑定顶点/索引缓冲区/拓扑类型、设置描述符表、绘制图元等，最后 SwapChain 呈现画面、交换前后缓冲区等
...
void BoxApp::Draw(const GameTimer& gt)
{
    ...
    // ID3D12CommandAllocator Reset 指示重新使用与命令分配器相关联的内存
    ...
    // Indicate a state transition on the resource usage.
    // Resource Barrier 标识 CurrentBackBuffer 切换为 Present 状态
	mCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
...
- DispatchMessage 该函数调度一个消息给窗口程序。通常调度从GetMessage取得的消息。消息被调度到的窗口程序即是MainProc()函数
...
- 在开启 4X MSAA 之后创建 SwapChain 会报错，原因是 DX12 不支持创建 4X MSAA 的 Swap Chain，在DX12当中图形驱动只负责将RenderingTarget当中的色彩图像拷贝到屏幕缓冲区进行显示，不再负责将一块支持MSAA的RenderingTarget（样本数大于屏幕实际可以渲染像素数）变换成一块非MSAA的RenderingTarget（样本数与屏幕实际可以渲染像素数相同）了

![](../images/2023.1.5/4.png)

- 解决方案可以是：1. 使用ID3D12GraphicsCommandList::ResolveSubresource方法。这个方法为我们提供了类似于DX11之前的缺省的Resolve方法，就是对MSAA的不同样本采用取平均值的方法得到最终像素颜色 2. 创建额外的PSO，在场景渲染完成之后，通过一个额外的全屏Quad的渲染启动一个用于计算MSAA Resolve的PS Shader，将MSAA RT的色彩缓冲区作为贴图输入（MSAA各个Sample的值会存储在贴图不同的MIP等级上），然后求所有MIP等级值的平均（当然也可以不是求平均，这里Resolve的算法可以根据需要任意定制）作为像素的最终颜色输出到non-MSAA RT
...
- [DirectX12（D3D12）基础教程（二）——理解根签名、初识显存管理和加载纹理、理解资源屏障](https://blog.csdn.net/u014038143/article/details/83067300)
- [在 Direct3D 12 中管理图形管道状态](https://learn.microsoft.com/zh-cn/windows/win32/direct3d12/managing-graphics-pipeline-state-in-direct3d-12)
- [Pipelines and Shaders with Direct3D 12](https://learn.microsoft.com/en-us/windows/win32/direct3d12/pipelines-and-shaders-with-directx-12)
- [4X MSAA fails](https://github.com/d3dcoder/d3d12book/issues/16)
- [从零开始手敲次世代游戏引擎（五十六）](https://zhuanlan.zhihu.com/p/37382556)